...
Serif体の書体としては{\usefont{T1}{ptm}{m}{n} Times}や{\usefont{T1}{pplx}{m}{n} Palatino}，{\usefont{OT1}{cmr}{m}{n} Computer Modern}などが挙げられます．
...
この節では文献リストのフィールド（\BibTeX{}における\ttauthor や\verb|title|）の並べ方について説明します．
...
\ttauthor や\verb|title|などのフィールドについての詳細は第~\ref{ssec:field}節を，各エントリー（\ttarticle や\ttbook など）ごとの詳細は第~\ref{ssec:article}節以降を参照してください．
\renewcommand\UrlFont{\rmfamily}
\begin{itembox}[l]{\textgt{\large 文献リスト作成の注意事項（全体）}}
    \begin{itemize}
        \item 並び順は原則として \\
            著者（\ttauthor）$\to$タイトル（\verb|title|）$\to$誌名（\ttjournal）・書名（\ttbooktitle）$\to$出版社（\verb|publisher|）・大学名（\verb|school|）・機関名（\ttinstitution）$\to$巻（\verb|volume|）$\to$号（\verb|number|）$\to$発行年（\verb|year|）$\to$ページ（\verb|pages|）$\to$論文番号・講演番号（\verb|note|） \\
            とする．
        ...
        \item 以上の内容が満たされていて，文献にアクセスするうえで十分な情報が書かれていれば月（\verb|month|）や章（\ttchapter）等は不要．
    \end{itemize}
\end{itembox}
...
\begin{itemize}
    ...
    \item \ttbooktitle \\
        ...
        例えば，\citet{Lueptow:Springer2000}はそれ単独でStability and experimental velocity field in Taylor--Couette flow with axial and radial flowという題目（\verb|title|）を持っていますが，これはPhysics of Rotating Fluidsという書籍（\ttbooktitle）の一部です．
    \item \ttchapter \\
    ...
    \item \verb|school| \\
        ...
        学部の卒業論文の場合は\hyperref[ssec:misc]{\ttmisc}に分類し，\verb|school|フィールドの代わりに\tthowpublished を使用してください．
    \item \verb|series| \\
        書籍のシリーズを入力します．
    ...
    \item \verb|url| \\
        文献やwebページのURLを入力します．
        \ttdoi フィールドと\verb|url|フィールドの両方に値が入っていた場合は\ttdoi の内容を優先してハイパーリンクを埋め込みます．
    \item \verb|volume| \\
    ...
    \item \verb|language|, \verb|langid| \\
        ...
        \verb|japanese|と書かれていると日本語文献として処理されますが，指定されていなくても基本的には自動で日本語文献を判別してくれます\footnote{\ttauthor や\texttt{title}などのフィールドに日本語が含まれていると\texttt{is.kanji.str\$}という機能で日本語文献と判断します．}．
...
\begin{screen}
    \begin{itemize}
        \item 必須項目 \\
        \ttauthor, \verb|title|, \ttjournal, \verb|year|
        \item オプション項目 \\
        \verb|volume|, \verb|number|, \verb|pages|, \verb|month|, \verb|note|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\ttjournal} (\colorbox[gray]{0.8}{\texttt{year}}), Vol.~\colorbox[gray]{0.8}{\texttt{volume}}, No.~\colorbox[gray]{0.8}{\texttt{number}}, pp.~\colorbox[gray]{0.8}{\texttt{pages}}, \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\ttjournal} (\colorbox[gray]{0.8}{\texttt{year}}), Vol.~\colorbox[gray]{0.8}{\texttt{volume}}, No.~\colorbox[gray]{0.8}{\texttt{number}}, pp.~\colorbox[gray]{0.8}{\texttt{pages}}, \colorbox[gray]{0.8}{\texttt{note}}.
...
\ttdoi と\verb|url|のフィールドは\jsmefile ではどちらも\ttjournal にハイパーリンクを埋め込む操作を行うのでどちらを使っても同様の結果を得られます．
\ttdoi も\verb|url|も無い場合は\ttjournal にハイパーリンクを埋め込みません．
...
\begin{screen}
    \begin{itemize}
        \item 必須項目 \\
        \ttauthor / \tteditor, \verb|title|, \verb|publisher|, \verb|year|
        \item オプション項目 \\
        \verb|volume|, \verb|number|, \verb|series|, \ttaddress, \ttedition, \verb|month|, \verb|note|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\texttt{publisher}} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\texttt{publisher}} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item \verb|bib|ファイル作成例 \vspace{-3mm}
\begin{verbatim}
...
\begin{screen}
    \begin{itemize}
        ...
        \verb|title|
        \item オプション項目 \\
        \ttauthor, \tthowpublished, \ttaddress, \verb|month|, \verb|year|, \verb|note|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\tthowpublished} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\tthowpublished} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item \verb|bib|ファイル作成例 \vspace{-3mm}
\begin{verbatim}
...
\begin{screen}
    \begin{itemize}
        \item 必須項目 \\
        \ttauthor / \tteditor, \verb|title|, \ttchapter / \verb|pages|, \verb|publisher|, \verb|year|
        \item オプション項目 \\
        \verb|volume|, \verb|series|, \ttaddress, \ttedition, \verb|month|, \verb|note|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\texttt{publisher}} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{pages}}, \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\texttt{publisher}} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{pages}}, \colorbox[gray]{0.8}{\texttt{note}}.
        \item \verb|bib|ファイル作成例 \vspace{-3mm}
\begin{verbatim}
...
\begin{screen}
    \begin{itemize}
        \item 必須項目 \\
        \ttauthor, \verb|title|, \ttbooktitle, \verb|year|
        \item オプション項目 \\
        \tteditor, \verb|pages|, \verb|organization|, \verb|publisher|, \ttaddress, \verb|month|, \verb|note|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\ttbooktitle}, \colorbox[gray]{0.8}{\texttt{publisher}} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{pages}}, \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\ttbooktitle}, \colorbox[gray]{0.8}{\texttt{publisher}} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{pages}}, \colorbox[gray]{0.8}{\texttt{note}}.
        \item \verb|bib|ファイル作成例 \vspace{-3mm}
\begin{verbatim}
...
上記の\citet{Lueptow:Springer2000}の例では，それ単独でStability and experimental velocity field in Taylor--Couette flow with axial and radial flowという題目（\verb|title|）を持っていますが，これはPhysics of Rotating Fluidsという書籍（\ttbooktitle）の一部です．
...
\begin{screen}
    \begin{itemize}
        ...
        \tteditor, \verb|pages|, \verb|organization|, \verb|publisher|, \ttaddress, \verb|month|, \verb|note|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\ttbooktitle} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\ttbooktitle} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item \verb|bib|ファイル作成例 \vspace{-3mm}
\begin{verbatim}
...
\begin{screen}
    \begin{itemize}
        ...
        \verb|title|
        \item オプション項目 \\
        \ttauthor, \verb|organization|, \ttaddress, \ttedition, \verb|month|, \verb|year|, \verb|note|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item \verb|bib|ファイル作成例 \vspace{-3mm}
\begin{verbatim}
@manual{Tecplot2023,
    author  = "{Tecplot, Inc.}",
...
\begin{screen}
    \begin{itemize}
        \item 必須項目 \\
        \ttauthor, \verb|title|, \verb|school|, \verb|year|
        \item オプション項目 \\
        \ttaddress, \verb|month|, \verb|note|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, Master's thesis, \colorbox[gray]{0.8}{\texttt{school}} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\texttt{school}}修士論文 (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item \verb|bib|ファイル作成例 \vspace{-3mm}
\begin{verbatim}
...
\begin{screen}
    \begin{itemize}
        ...
        無し
        \item オプション項目 \\
        \ttauthor, \verb|title|, \tthowpublished, \ttarchivePrefix, \tteprint, \verb|month|, \verb|year|, \verb|note|, \ttkey
        \item 出力（通常，英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\tthowpublished} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（通常，日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\tthowpublished} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（arXivの場合） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, arXiv: \colorbox[gray]{0.8}{\tteprint} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
...
ただし，\ttmastersthesis や\ttphdthesis と異なり，\verb|school|のフィールドを使用できないので\tthowpublished で代用します．
...
\begin{screen}
    \begin{itemize}
        \item 使用可能項目 \\
        \ttauthor, \verb|title|, \tthowpublished, \verb|month|, \verb|year|, \verb|url|, \ttdoi, \ttaccess, \verb|note|
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, available from \textless\colorbox[gray]{0.8}{\texttt{url}}\textgreater, (accessed on \colorbox[gray]{0.8}{access}).
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \textless\colorbox[gray]{0.8}{\texttt{url}}\textgreater, (参照日 \colorbox[gray]{0.8}{access}).
        \item \verb|bib|ファイル作成例 \vspace{-3mm}
\begin{verbatim}
...
\begin{screen}
    \begin{itemize}
        ...
        \ttaddress, \verb|month|, \verb|note|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, Ph.D. dissertation, \colorbox[gray]{0.8}{\texttt{school}} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\texttt{school}}博士論文 (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item \verb|bib|ファイル作成例 \vspace{-3mm}
\begin{verbatim}
@phdthesis{塚原:博論2007,
    author  = {塚原, 隆裕},
...
\begin{screen}
    \begin{itemize}
        ...
        \item オプション項目 \\
        \tteditor, \verb|organization|, \verb|publisher|, \ttaddress, \verb|month|, \verb|note|, \ttkey
        \item 出力 \\
...
\begin{screen}
    \begin{itemize}
        \item 必須項目 \\
        \ttauthor, \verb|title|, \ttinstitution, \verb|year|
        \item オプション項目 \\
        \verb|type|, \verb|number|, \ttaddress, \verb|month|, \verb|note|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\ttinstitution} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, \colorbox[gray]{0.8}{\ttinstitution} (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item \verb|bib|ファイル作成例 \vspace{-3mm}
\begin{verbatim}
...
\begin{screen}
    \begin{itemize}
        \item 必須項目 \\
        \ttauthor, \verb|title|, \verb|note|
        \item オプション項目 \\
        \verb|month|, \verb|year|, \ttkey
        \item 出力（英語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors} and \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
        \item 出力（日本語文献） \\
            \colorbox[gray]{0.8}{\ttauthorf}, \colorbox[gray]{0.8}{\ttauthors}, \colorbox[gray]{0.8}{\ttauthort}, \colorbox[gray]{0.8}{\texttt{title}}, (\colorbox[gray]{0.8}{\texttt{year}}), \colorbox[gray]{0.8}{\texttt{note}}.
...
\end{itemize}
次に本文の\verb|tex|ファイルの設定ですが，おおまかに
\begin{screen}
\begin{verbatim}
\documentclass[a4paper,fleqn,uplatex,dvipdfmx]{jsarticle}
%%% hyperref
...
ここでは「文献」に相当するものが節なので\verb|section|と書いていますが，章に相当する場合は\ttchapter に変えてあげてください．
