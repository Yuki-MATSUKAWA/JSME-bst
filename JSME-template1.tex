...
\begin{screen}
    \begin{itemize}
        ...
        無し
        \item オプション項目 \\
        \verb|author|, \verb|title|, \verb|howpublished|, \verb|month|, \verb|year|, \verb|archivePrefix|, \verb|eprint|, \verb|note|, \verb|key|
        \item 理想形（通常） \\
...
英語のwebページの場合は末尾に
\begin{quote}
    , available from \textless URL \textgreater, (accessed on 参照日).
\end{quote}
を入れることが日本機械学会の原稿テンプレートに記載されています．
ただ，日本語のwebページの場合の書き方が不明なので\verb|jsme.bst|では
\renewcommand\UrlFont{\rmfamily}
\begin{quote}
